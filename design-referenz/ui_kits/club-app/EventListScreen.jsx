		...
		<button className={'ev-card' + (past ? ' ev-card--past' : '')} onClick={() => onOpen(e.id)}>
			...
			<div className="ev-card__body">
				...
				<div className="ev-card__foot">
					...
					{past && e.present != null ? (
						<span className="ev-card__att" title="Erfasste Anwesenheit">
							<Ic name="user-check" size={14} /> {e.present} anwesend
						</span>
					) : (
	...
	return (
		<div className="screen">
			<AppBar
				...
				trailing={
					<IconButton
						icon={<Ic name={calendar ? 'list' : 'calendar'} size={22} />}
			...
			{!calendar && (
				<div className="screen__pad" style={{ paddingTop: 4, paddingBottom: 12 }}>
					<SegmentedControl
						...
						onChange={setPhase}
						options={[
							{ label: 'Anstehend', value: 'upcoming' },
			...
			<div className="screen__scroll">
				{calendar ? (
					<div className="screen__pad">
						<CalendarView events={events} onOpen={onOpen} />
					</div>
				) : (
					<React.Fragment>
						{groups.map(([month, items]) => (
							<div key={month} className="ev-group">
								...
								<div className="card-list">
									{items.map((e) => (
										<EventCard key={e.id} e={e} onOpen={onOpen} onResponses={onResponses} />
									))}
								</div>
							</div>
						))}
						{list.length === 0 && (
							<div className="ev-empty">
								Keine {phase === 'past' ? 'vergangenen' : 'anstehenden'} Termine.
							</div>
						)}
					</React.Fragment>
				)}
	...
	return (
		<div>
			<div className="cal-nav">
				<button className="cal-nav__btn" aria-label="Vorheriger Monat" onClick={() => step(-1)}>
					<Ic name="chevron-left" size={20} />
				</button>
				<div className="cal-nav__title">
					{MONTHS_DE[cursor.m]} {cursor.y}
				</div>
				<button className="cal-nav__btn" aria-label="Nächster Monat" onClick={() => step(1)}>
					...
					>
						...
						{evByDay[d] && (
							<span
								...
								style={{ background: `var(--status-${evByDay[d].status}-dot)` }}
							/>
						)}
					</div>
				))}
			...
			<div className="card-list" style={{ marginTop: 16 }}>
				{monthEvents.length === 0 && (
					<div className="ev-empty" style={{ padding: '20px 0' }}>
