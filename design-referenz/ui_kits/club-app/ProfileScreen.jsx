		...
		<div className="screen">
			<AppBar
				...
				leading={
					onBack ? (
						<IconButton
							icon={<Ic name="chevron-left" size={22} />}
							label="Zurück"
							onClick={onBack}
						/>
					) : null
				...
				trailing={
					<IconButton
						icon={<Ic name={edit ? 'check' : 'pencil'} size={20} />}
						label="Bearbeiten"
						tone="primary"
				...
				<div className="screen__pad detail__stack">
					...
					<Card>
						<div className="form-stack">
							<Input
								...
								disabled={!edit}
								icon={<Ic name="mail" size={20} />}
							/>
							<Input
								label="Handy"
								defaultValue={me.phone}
								disabled={!edit}
								icon={<Ic name="phone" size={20} />}
							/>
							<Input
								label="Geburtstag"
								defaultValue="14.03.1961"
								disabled={!edit}
								icon={<Ic name="cake" size={20} />}
							/>
						</div>
					</Card>
					...
					<Card>
						<div className="switch-stack">
							...
							<div className="switch-row">
								<span>E-Mail-Fallback</span>
								<Switch defaultChecked />
							</div>
							<div className="switch-row">
								<span>Geburtstags-Hinweise</span>
								<Switch />
							</div>
						</div>
					</Card>
					...
					{edit && (
						<Button variant="primary" size="lg" fullWidth onClick={() => setEdit(false)}>
							Änderungen speichern
						</Button>
					)}
					<button className="logout">
						<Ic name="log-out" size={18} /> Ausloggen
					</button>
					<div style={{ height: 12 }} />
				</div>
			</div>
