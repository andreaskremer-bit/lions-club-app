	...
	return (
		<div className="qf-num">
			<button
				...
				onClick={() => set(v - 1)}
			>
				<Ic name="minus" size={18} />
			</button>
			<div className="qf-num__val">
				<span className="u-mono">{v}</span>
				{unit ? <span className="qf-num__unit">{unit}</span> : null}
			</div>
			<button
				type="button"
				className="qf-num__btn"
...
function QuestionField({ q, value, onChange }) {
	...
	if (q.kind === 'single') {
		...
		control = (
			<NumberStepper value={value} onChange={onChange} min={q.min} max={q.max} unit={q.unit} />
		);
	} else {
		// text
		control = (
	...
	return (
		<div className="qf">
			<div className="qf__label">
				{q.icon ? (
					<span className="qf__ic">
						<Ic name={q.icon} size={16} />
					</span>
				) : null}
				<span>
					{q.label}
					{q.required ? <span className="qf__req">*</span> : null}
				</span>
			</div>
			{control}
		</div>
