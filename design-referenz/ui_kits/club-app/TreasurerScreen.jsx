...
function TreasurerScreen({ onBack }) {
  ...
  return (
    <div className="screen">
      <AppBar center bordered title="Auswertung"
        leading={<IconButton icon={<Ic name="chevron-left" size={22} />} label="Zurück" onClick={onBack} />}
        trailing={<IconButton icon={<Ic name="download" size={20} />} label="Export" tone="primary" />} />

      <div className="screen__scroll">
        <div className="screen__pad detail__stack" style={{ paddingTop: 14 }}>
          <Select label="Zeitraum" options={['Clubjahr 2025/26', '1. Halbjahr 2026', 'Juni 2026']} defaultValue="Clubjahr 2025/26" />
          ...
          <div className="treas-totals">
            ...
            <Card>
              ...
              <div className="treas-stat__n u-mono" style={{ color: 'var(--gold-700)' }}>{euro(totalSum)}</div>
            </Card>
          </div>
          ...
          <div className="treas-note">
            <Ic name="info" size={15} /> Die App erfasst nur Abwesenheiten. Die Buchung erfolgt extern durch den Schatzmeister.
          </div>

          <div className="treas-actions">
            <Button variant="secondary" iconLeft={<Ic name="file-text" size={18} />}>PDF</Button>
            <Button variant="secondary" iconLeft={<Ic name="table" size={18} />}>CSV</Button>
          </div>
          <div style={{ height: 12 }} />
        </div>
      </div>
