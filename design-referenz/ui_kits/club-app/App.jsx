						...
						<Card key={p.id}>
							<div className="news__head">
								...
								<div className="news__by">
									<span className="news__author">{p.author}</span>
									<span className="news__meta">
										{p.role} · {p.time}
									</span>
								</div>
								<Tag tone="blue" outline>
									{p.tag}
								</Tag>
							</div>
			...
			<div className="screen__scroll">
				<div className="screen__pad detail__stack" style={{ paddingTop: 6 }}>
					<Card interactive onClick={() => onNav('profile')} className="more__profile">
						...
						<div>
							<div className="more__name">{me.name}</div>
							<div className="more__role">{me.role} · Profil bearbeiten</div>
						</div>
					...
					<Card flush>
						...
						<ListRow
							...
							subtitle="Abwesenheiten &amp; Beiträge"
							chevron
					...
					<Card flush>
						...
						<ListRow
							lead={lead('image')}
							title="Galerie"
							subtitle="Google-Share öffnen"
							trailing={<Ic name="external-link" size={17} color="var(--text-muted)" />}
						/>
...
function App() {
	...
	if (overlay?.type === 'event')
		...
		body = <AttendanceScreen onBack={() => setOverlay(null)} />;
	else if (overlay?.type === 'treasurer')
		body = <TreasurerScreen onBack={() => setOverlay(null)} />;
	else if (overlay?.type === 'admin')
		body = (
			<AdminScreen
				onBack={() => setOverlay(null)}
				onOpen={(id) => setOverlay({ type: 'event', id })}
				onResponses={showResponses}
			/>
		);
		...
		<PhoneFrame>
			<div className="app">
				...
				<ResponseSheet
					...
					onStatus={(s) => setSheet((x) => ({ ...x, status: s }))}
				/>
			</div>
		</PhoneFrame>
	);
