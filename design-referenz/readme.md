...
- **`Icon set: `[`Lucide`](https://lucide.dev)**`— line icons, ~1.9 stroke weight, rounded caps. Loaded via`**`CDN`**`(unpkg.com/lucide) in cards and the UI kit. Chosen to match the brief's „Linien-Icons" for the tab bar and quiet, modern feel.`_`(Substitution flag: the spec doesn't name an icon library — Lucide is a close, freely-licensed match. Swap if the club standardises on another set.)`_
